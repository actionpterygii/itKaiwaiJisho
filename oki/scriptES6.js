...
class State {
    ...
    changeDarkMode(private_darkMode_flg = this.darkMode_flg) {
        // ダークモードになっていたら
...
class Jisho {
    // newされたときにする
    constructor(jisho_data_path, krngJisho_instance_name) {
        ...
        const _this = this;
        // パスからよむよ辞書データ
        const my_xhr = new XMLHttpRequest();
    ...
    containing(item, input_text, exact_match) {
        // スペースを削除する
            ...
            // 完全一致検索じゃなかったら
                ...
                for (const key in this.jisho_data) {
                    ...
                    for (const item_key in this.word_items) {
                        // 単語の中の一つの項目のキー
                        const item = this.word_items[item_key];
            ...
            if (element[key] !== '') {
                ...
                switch (key) {
                    ...
                    case 'krng':
                        ...
                        const random_id = 'random_' + Math.random().toString(32).substring(2);
                        html +=
                            `<div class="${key}">` +
                                // ここだめ、はやくしゅうしゅうせいしよう
    ...
    // 入力から結果(HTML)を返す
    createResult(input_text) {
        // 最後かえす文字列
        let entity = '';
        // 必要な要素を選定する
        const required_elements = this.search(input_text, false);
        // 返すべき結果があれば
    ...
    createRandomOneTangoResult() {
        // 単語の数内でランダムにただひとつの番号を得ました
        const random_num = Math.floor(Math.random() * this.tango_quantity);
        // ランダム番号からいち単語取得
        const random_tango = this.jisho_data[random_num];
    ...
    createKanrengo(krng_facade) {
        // 押された開くボタンで開く要素(それは次の次にある要素)
        const krng_contents = krng_facade.nextElementSibling.nextElementSibling;
        // 押されたボタンで必要なの単語を取り出し1つずつ配列に入れる
        const tangos = krng_facade.getAttribute('value').split(',');
...
////////////////////
// HTML要素
const all_area = document.getElementById('wrap');
const input_area = document.getElementById('input_area');
const result_area = document.getElementById('result_area');
const darkMode_btn = document.getElementById('darkMode_btn');
const quickSearch_btns = document.getElementsByClassName('quickSearch_btn');
const qrcode_btn = document.getElementById('qrcode_btn');
const nyuryoku_btn = document.getElementById('nyuryoku_btn');
const random_btn = document.getElementById('random_btn');
const guguru_btn = document.getElementById('guguru_btn');
// CSS要素
const style_sheet = document.documentElement.style;
// ぐぐりにつかうURL
const guguru_url = 'https://www.google.com/search?q=';
// これからの状態をもつもの
const state = new State();
// 辞書というモノはいまはこのひとつ
const jisho = new Jisho('jisho.json', 'jisho');
////////////////////
// イベント登録
////////////////////
// HTMLパースが終わってから発火
...
qrcode_btn.addEventListener('click', function () {
    // 次の要素(QRコード画像)取得
    const qrcode_img = qrcode_btn.nextElementSibling;
