...
- 新パソコンで`npm install`したらなぜか`gulp-sass`がはいらず個別に入れ直す
  - したら`^4.0.2`から`^4.1.1`になっていた
  - そのあと、なんか`npm update`を促される
- pythonをコマンドで実行すると`python`とだけ表示される謎の現象
  - Windowsの設定 → アプリと機能 → アプリ実行エイリアス からpythoの付いてるのを無効にすればok
    - なにこれまじで
